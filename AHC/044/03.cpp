...
tuple<double, city_group, city_group> swap_city(city_group a, int ai, city_group b, int bi, vector<double>& x, vector<double>& y) { 
  ...
  auto compute_mst = [&](const vector<int> &cities) -> pair<double, vector<pair<int, int>>> {
    ...
    double total = 0.0;
    vector<pair<int, int>> mst_edges(n-1);
    ...
    while(sz--) {
      ...
      seen.at(v) = true;
      total += mi;
      if(pv.at(v)!=-1) mst_edges.at(sz) = pair(cities.at(v),cities.at(pv.at(v)));
      rep(i,n) {
        double dist = hypot(x.at(cities.at(v))-x.at(cities.at(i)),y.at(cities.at(v))-y.at(cities.at(i)));
        if(chmin(cost.at(i),dist)) pv.at(i) = v;
      }
    }
...
vector<city_group> simulated_annealing(vector<city_group> ans, vector<double>& x, vector<double>& y) {
  // 初期解のスコア計算
  double cur_score = 0.0;
  for (auto g : ans) for (auto [u, v] : g.edges) cur_score += hypot(x.at(u) - x.at(v), y.at(u) - y.at(v));
  
  auto cur_ans = ans;
  uniform_int_distribution<int> dist(0,ans.size()-1);

  // 温度スケジュールのパラメータ設定
  double T0 = 1000.0;    // 初期温度
  double T_end = 1e-4;     // 最低温度

  int tot = 0;
  int upd = 0;
  while (true) {
    tot++;
    // タイマーがTIME_LIMITを超えたら終了
    if (timer.get_time() > TIME_LIMIT) break;

    // 現在の経過時間に基づく温度更新（指数関数的減衰）
    double t = timer.get_time();
    // より緩やかな減衰にする
    double T = T0 * pow(T_end / T0, t / TIME_LIMIT * 0.7); // 0.7など係数を調整

    // 2つのグループをランダムに選択
    int i = dist(mt) % cur_ans.size();
    int j = dist(mt) % cur_ans.size();
    if(i == j) continue;

    // それぞれのグループ内のランダムな都市を選び、入れ替えた場合の変化量 diff と新たなグループを取得
    auto [diff, new_group_i, new_group_j] = swap_city(cur_ans.at(i), dist(mt) % cur_ans.at(i).cities.size(),
                                                     cur_ans.at(j), dist(mt) % cur_ans.at(j).cities.size(), x, y);

    double new_score = cur_score + diff;
  
    // 改善なら無条件採用、悪化なら確率的に採用
    uniform_real_distribution<double> udist(0.0, 1.0);
    if (diff < 0 || exp(-diff / T) > udist(mt)) {
      upd++;
      cur_ans.at(i) = new_group_i;
      cur_ans.at(j) = new_group_j;
      cur_score = new_score;
    }
    if (diff < 0) cerr << "improve: " << diff << "\n";
    else if (exp(-diff / T) > udist(mt)) cerr << "accept worsen: " << diff << " at T=" << T << "\n";
...
int main() {
  ...
  if(m>1) ans = simulated_annealing(ans, x, y);
