...
vector<city_group> climbing(vector<city_group> ans, vector<double>& x, vector<double>& y) {
  ...
  while(1) {
    ...
    int i = dist(mt);
    uniform_int_distribution<int> dist2(i+1,ans.size());
    int j = dist(mt);
    if(i==j) continue;
    uniform_int_distribution<int> dist3(0,cur_ans.at(i).cities.size());
    uniform_int_distribution<int> dist4(0,cur_ans.at(j).cities.size());
    auto [diff, a, b] = swap_city(cur_ans.at(i),dist3(mt)%cur_ans.at(i).cities.size(),cur_ans.at(j),dist4(mt)%cur_ans.at(j).cities.size(),x,y);
