...
struct LazySegTree{
	...
	LazySegTree(int size){
		this->size = size;
		int L = 1<<size;
		seg.resize(L*2), delay.resize(L*2);
		l.resize(L*2), r.resize(L*2);
		for(int i = 0; i < L; i++) l[L+i] = i, r[L+i] = i;
		for(int i = L-1; i >= 1; i--) l[i] = l[i*2], r[i] = r[i*2+1];
	}
	...
	T opeT(T a, T b){ //
		return T(a.fi*b.fi, a.se*b.fi + b.se);
	}
	S act(S a, T d){ //
		return d.fi*0 + d.se;
	}
	...
	void setup(){
		for(int i = (1<<size)-1; i >= 1; i--) seg[i] = opeS(seg[i*2], seg[i*2+1]);
	}

	void eval(int k){
		if(delay[k] != idT()){
			seg[k] = act(seg[k], delay[k]);
			if(l[k] < r[k]){
				delay[k*2] = opeT(delay[k*2], delay[k]);
				delay[k*2+1] = opeT(delay[k*2+1], delay[k]);
			}
			delay[k] = idT();
		}
	}
	...
	void update(int i, S val){
		...
		for(int j = 0; j < size; j++){
			k >>= 1;
			eval(k*2), eval(k*2+1);
			seg[k] = opeS(seg[k*2], seg[k*2+1]);
		}
	}

	void add(int a, int b, int k, T val){
		eval(k);
		if(b < l[k] || r[k] < a) return;
		if(a <= l[k] && r[k] <= b){
			delay[k] = opeT(delay[k], val);
			eval(k);
			return;
		}
		add(a, b, k*2, val);
	...
	S query(int a, int b, int k){
		eval(k);
		if(b < l[k] || r[k] < a) return idS();
		if(a <= l[k] && r[k] <= b) return seg[k];
		S lval = query(a, b, k*2), rval = query(a, b, k*2+1);
		return opeS(lval, rval);
	}
	S query(int a, int b){
		if(a > b) return idS();
		return query(a, b, 1);
...
int main(void)
{
	...
	rep(i, 1, m){
		cin >> l >> r >> x;
		ll L = r-l+1;
		mint inv = mint(L).inverse();
		seg.add(l, r, {mint(L-1)*inv, mint(x)*inv});
	}
	rep(i, 1, n){
		cout << seg.query(i, i);
		if(i < n) cout << " ";
